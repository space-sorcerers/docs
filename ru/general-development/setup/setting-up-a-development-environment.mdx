...
2. Откройте раздел Extensions и установите следующие расширения:
   ...
   | Расширение | Для чего |
   |---|---|
   | [C# Dev Kit](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csdevkit) | Основной набор для C#: автодополнение, навигация, отладка, управление проектами. Включает C# и .NET Install Tool. |
   | [EditorConfig](https://marketplace.visualstudio.com/items?itemName=EditorConfig.EditorConfig) | Единый стиль форматирования кода во всём проекте. |
   | [GitLens](https://marketplace.visualstudio.com/items?itemName=eamodio.gitlens) | Просмотр истории, авторов строк, сравнение версий прямо в редакторе. |
   | [GitHub Pull Requests](https://marketplace.visualstudio.com/items?itemName=GitHub.vscode-pull-request-github) | Создание и ревью PR не выходя из VS Code. |
   | [Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python) и [Python Debugger](https://marketplace.visualstudio.com/items?itemName=ms-python.debug-py) | Подсветка, автодополнение и отладка Python-скриптов (нужны для вспомогательных инструментов). |
   | [Robust YAML](https://marketplace.visualstudio.com/items?itemName=slava0135.robust-yaml) | Подсветка и автодополнение для YAML-прототипов Robust Toolbox. Требует [YAML Language Support](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml). |
...
[SS14 Editor](https://github.com/TheShuEd/SS14Editor) от TheShuEd — визуальный редактор YAML-прототипов для любых форков SS14. Укажите путь к проекту — редактор прочитает собранные сборки игры и предоставит:
...
- **Desktop-приложение** (рекомендуется): готовые сборки Electron для Windows (NSIS-установщик / portable), Linux (AppImage), macOS (Apple Silicon DMG) на [странице релизов](https://github.com/TheShuEd/SS14Editor/releases).
