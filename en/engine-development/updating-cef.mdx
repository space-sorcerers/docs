...
Clone [`build-dependencies`](https://github.com/space-wizards-federation/build-dependencies). Extract your downloaded CEF copies (for both platforms) into `natives/cef/` so it looks like this: 
...
Specifically, test OpenDream on Windows, Linux, and both again through the launcher. You can test the launcher by following [Testing against launcher](./testing-against-launcher). Just use `Tools/package_webview.py` to package the zip for it.
