...
By default, this HTTP API listens on the same port number as the game UDP port, but this can be changed with the relevant `status.*` [CVars](../general-development/tips/config-file-reference). 
...
For details about build information, see [Advanced Server Build Configuration](../general-development/setup/server-hosting-tutorial#server-build-configuration).

Content can also edit this at will.

## Watchdog API

Requires authorization by specifying the `WatchdogToken` HTTP header to match the `watchdog.token` CVar (automatically set when using the [watchdog](../server-hosting/setting-up-ss14-watchdog)).
