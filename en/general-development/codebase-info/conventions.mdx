...
See [Codebase Organization](./codebase-organization) for guidelines on how files and folders are organized in the SS14 codebase.

Read the [Pull Request guidelines](./pull-request-guidelines) to learn how to make your code more reviewable by maintainers.
...
<Accordion title="C# code example (click to expand)">
```csharp
[DataField]
...
<Accordion title="YAML prototype example (click to expand)">
```yaml
...
<Accordion title="RSI meta.json (click to expand)">
...
<Accordion title="Admin log with entities example (click to expand)">
```csharp
...
### Proxy Methods
When possible, try using the `EntitySystem` [proxy methods](https://github.com/space-wizards-federation/RobustToolbox/blob/master/Robust.Shared/GameObjects/EntitySystem.Proxy.cs) instead of using the `EntityManager` property.

<Accordion title="Examples (click to expand)">
```csharp
...
<Accordion title="Example of what not to do (click to expand)">
```csharp
...
<Accordion title="Example of what to do (click to expand)">
```csharp
void DoSomething(EntityUid otherEntity)
{
...
<Accordion title="Example usage (click to expand)">
```csharp
