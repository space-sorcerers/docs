...
* [Git](https://git-scm.com/) or one of the [many](https://www.sourcetreeapp.com/) [third-party](http://www.syntevo.com/smartgit/) [UIs](https://tortoisegit.org/) that make it easier to use. Make sure to let it install to your PATH like [this](/images/setup/git-path.png).
* [Python 3.7 or higher](https://www.python.org/). Make sure to install it into your [PATH on Windows](/images/setup/python-path.png). Also make sure the 'py launcher' option is enabled when installing on Windows. You should get python from [python.org](https://www.python.org/). Versions installed from the windows store sometimes cause build issues.
...
If you're **unfamiliar with Git**, or just don't know how to proceed, follow the [Git for the SS14 Developer](./git-for-the-ss14-developer) guide, which goes in depth on how to contribute to the game and how to set up your initial repository. It also touches on submodule setup, but that's included here as well because of its importance.
...
2. Open the Extensions panel and install the following extensions:
   ...
   | Extension | Purpose |
   |---|---|
   | [C# Dev Kit](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csdevkit) | Core C# tooling: autocompletion, navigation, debugging, project management. Includes C# and .NET Install Tool. |
   | [EditorConfig](https://marketplace.visualstudio.com/items?itemName=EditorConfig.EditorConfig) | Consistent code formatting across the project. |
   | [GitLens](https://marketplace.visualstudio.com/items?itemName=eamodio.gitlens) | Inline blame annotations, history, code lens — see who changed what and why. |
   | [GitHub Pull Requests](https://marketplace.visualstudio.com/items?itemName=GitHub.vscode-pull-request-github) | Create, review, and manage PRs without leaving the editor. |
   | [Python](https://marketplace.visualstudio.com/items?itemName=ms-python.python) and [Python Debugger](https://marketplace.visualstudio.com/items?itemName=ms-python.debug-py) | Syntax highlighting, autocompletion, and debugging for Python helper scripts. |
   | [Robust YAML](https://marketplace.visualstudio.com/items?itemName=slava0135.robust-yaml) | Syntax highlighting and autocompletion for Robust Toolbox YAML prototypes. Requires [YAML Language Support](https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml). |

3. Open the repo folder via `File > Open Folder`. VS Code will prompt you to select the `.slnx` file — accept it.
...
[SS14 Editor](https://github.com/TheShuEd/SS14Editor) by TheShuEd is a standalone visual editor for prototype YAML files. It works with any SS14 fork — point it at your project folder and it reads the compiled game assemblies to provide:

- Auto-complete for entity IDs, component names, `!type` polymorphic types
- Display of all prototype and component fields, including XML doc summaries from the source code
- Inheritance-aware view — see which components a child entity inherits from its parents
- Support for complex prototypes like `EntityTable`, `ConstructionGraph`, `HTN`

The editor reads game DLLs via `System.Reflection.MetadataLoadContext` — no game code is ever executed.

### Downloads

- **Desktop app** (recommended): pre-built Electron packages for Windows (NSIS installer / portable), Linux (AppImage), macOS (Apple Silicon DMG) on the [Releases page](https://github.com/TheShuEd/SS14Editor/releases).
- **CLI / browser**: standalone self-contained binary that runs an HTTP server on `http://localhost:2701`. Open it in any browser.

### Requirements

- .NET 10 SDK (if building from source)
- Any SS14 fork with compiled assemblies
