...
<Danger title="DO NOT USE GITKRAKEN">
...
<Danger title="Name and Email privacy">
...
<Info>
If you plan to use SSH instead of HTTPS for cloning (e.g. to avoid authentication issues), set up an SSH key:
...
Then copy `~/.ssh/id_ed25519.pub` and add it in GitHub → Settings → SSH and GPG keys. Verify with `ssh -T git@github.com`.
</Info>
...
Navigate to the [Space Station 14 repository](https://github.com/space-wizards-federation/space-station-14) and click here:
...
Then, we'll enter the command for cloning **our** remote repository--not the `space-wizards-federation/space-station-14` repository.
...
One issue: we don't have a reference to the original `space-wizards-federation/space-station-14` remote repository anywhere! How are we supposed to update our local repository without it? So let's make sure we've navigated inside our local repo's folder, and we'll add a new remote:
...
All this does is add a new remote named `upstream` that points to the original `space-wizards-federation/space-station-14` repository. Now we can receive updates from the main repository whenever we want! (see below on how to do that). 
...
```admonish warning title="Before working on your first PR to the space-wizards-federation repo"
Please make sure you read through the [Freezes & Restrictions](https://github.com/space-wizards-federation/space-station-14/issues/8524) and ensure your idea does not fall into the freezes or if your PR requires some prerequisite before being made. 
```
...
![sgstaging1.png](/images/general-development/setup/git-for-the-ss14-developer/tgstaging1.png)
...
If you only need some commits from one branch into another (e.g. your feature branch accidentally includes commits from an old branch), cherry-pick them individually:
...
Find the hashes with `git log --oneline`. The commits are applied on top of your current branch without bringing unrelated history.

## Undoing an accidental pull

If you pulled into the wrong branch and haven't pushed yet, use `git reflog` to go back:

```
git reflog
# find the entry before the pull, e.g. HEAD@{3}
git reset --hard HEAD@{3}
```

If you already pushed, create a clean branch and cherry-pick your intended commits instead.
